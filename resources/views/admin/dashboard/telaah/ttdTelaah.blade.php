...
@section('breadcrump')
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Data Tanda Tangan Telaah</li>
          </ol>
@stop
@section('content')
...
<div class="box box-solid box-warning">
  ...
  <div class="box-body">
    <p class="warning">Data Penandatangan Telaah ini akan digunakan sebagai Referensi ketika membuat Telaah. mohon pastikan data Kepala Unit Kerja Penelaah telah di masukkan.</p>
  </div><!-- /.box-body -->
</div><!-- /.box -->
	<div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="box-body">
              <form  role="form" method="POST" action="{{ url('ttdTelaah/tambah') }}">
                @csrf
	...
	<div class="modal modal-default fade" id="setDefault">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="box-body">
              <form  role="form" method="POST" action="{{ url('ttdTelaah/setDefault') }}">
                @csrf
                  ...
                  <table class="" width="100%">
                    <tr>
                      ...
                      <td width="70%">
                    	   <select id="id_ttd_telaah" class="form-control" name="id_ttd_telaah" style="width:100%" required>
                              <option value=""> -- Pilih Penandatangan --</option>
                              @foreach($data as $list) 
                                  <option value="{{ $list->id_ttd_telaah}}">{{ $list->nama_penelaah }} - {{ $list->jabatan }}</option>
                              @endforeach
                          </select>
                          <small class="help-block"></small>
                          <script>
                               $('#id_ttd_telaah').selectize({
                                create: false,
                                sortField: 'text'
                               });
                          </script>
                ...
                <div class="box-body">
                	<table id="dataTtd" class="table table-bordered">
                		...
                		<tbody>
                			...
                			<tr>
                				<td>{{$urut++}}</td>
                				<td>{{$ttd->nama_penelaah}}</td>
                				<td>{{$ttd->nip_penelaah}}</td>
                				<td>{{$ttd->jabatan}}</td>
                				<td>
                					@if($ttd->status == 1) 
                				...
                				<td>
                					<div class="btn-group">
                						<a data-toggle="modal" data-target="#edit{{$ttd->id_ttd_telaah}}" class="btn btn-primary btn" title="Edit Data"><i class="fa fa-edit"></i></a>
                						<a data-toggle="modal" data-target="#hapus{{$ttd->id_ttd_telaah}}" class="btn btn-danger btn" title="Hapus Data"><i class="fa fa-trash"></i></a>
                					</div>
                					
                          <!-- Modal Edit -->
                        <div class="modal modal-default fade" id="edit{{$ttd->id_ttd_telaah}}">
                            <div class="modal-dialog">
                              <div class="modal-content">
                                <div class="modal-header">
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                ...
                                <div class="modal-body">
                                  <div class="box-body">
                                    <form  role="form" method="POST" action="{{ url('ttdTelaah/edit/'.$ttd->id_ttd_telaah.'') }}">
                                      @csrf
                                        ...
                                        <table class="" width="100%">
                                          <tr>
                                            <td width="30%"><b>Nama Kepala</b></td>
                                            <td width="70%">
                                              <input id="nama_kepala" type="text" class="form-control {{ $errors->has('nama_kepala') ? ' is-invalid' : '' }}" name="nama_kepala" value="{{ $ttd->nama_penelaah }}" placeholder="Nama Kepala" required autofocus style="width:100%">
                                              <input type="hidden" name="id_unit_kerja" value="{{$ttd->id_ttd_telaah}}">
                                              <small class="help-block"></small>
                                          ...
                                          <tr>
                                            <td><b>NIP</b></td>
                                            <td>
                                              <input id="nip_kepala" type="text" class="form-control {{ $errors->has('nip_kepala') ? ' is-invalid' : '' }}" name="nip_kepala" value="{{ $ttd->nip_penelaah }}" required autofocus placeholder="NIP Kepala" style="width:100%">
                                              <small class="help-block"></small>
                          ...
                          -->
                            ...
                            <!-- modal untuk hapus -->
                            <div class="modal modal-warning fade" id="hapus{{$ttd->id_ttd_telaah}}">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  ...
                                  <div class="modal-body" style="color:#ffffff;">
                                    <form method="POST" action="{{ url('/ttdTelaah/hapus/'.$ttd->id_ttd_telaah.'') }}">
                                      @csrf
                                      <input type="hidden" name="id" id="id" value="{{ $ttd->id_ttd_telaah }}">
